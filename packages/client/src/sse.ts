import { Message, MessageType, type ParsedMessage } from "@mocky-balboa/websocket-messages";
...
export class SSE {
  ...
  constructor(private readonly requestId: string, private readonly sendMessage: (message: Message<MessageType, Extract<ParsedMessage, { type: MessageType }>["payload"]>) => void) {}
  ...
  dispatchEvent(event: string, data: string) {
    ...
    this.sendMessage(new Message(MessageType.SSE_EVENT, { id: this.requestId, event, data }))
  }
   ...
   */
  close() {
    this.sendMessage(new Message(MessageType.SSE_CLOSE, { id: this.requestId }))
  }
  ...
  error(status: number, body: string) {
    ...
    this.sendMessage(new Message(MessageType.SSE_ERROR, { id: this.requestId, status, body }))
  }
}
