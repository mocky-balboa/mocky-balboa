...
describe("GraphQLRoute", () => {
	...
	describe("constructor and property getters", () => {
		test("should construct GraphQLRoute with correct properties", () => {
			...
			expect(graphqlRoute.query).toBe(
				"query GetUser($id: ID!) { user(id: $id) { id name } }",
			);
		});
	...
	describe("fulfill method", () => {
		...
		describe("header management", () => {
			...
			test("should preserve custom headers while setting content-type", () => {
				...
				expect(result.response.headers.get("Authorization")).toBe(
					"Bearer token",
				);
			});
	...
	describe("edge cases and error scenarios", () => {
		...
		test("should handle special characters in strings", () => {
			...
			expect(specialRoute.variables.text).toBe(
				"Special chars: áéíóú ñ ç 中文 русский العربية 🎉",
			);
		});
		...
		test("should handle GraphQLError serialization edge cases", async () => {
			...
			(
				errorWithCircularRef as GraphQLError & { customProp: unknown }
			).customProp = {
				circular: errorWithCircularRef,
			};
