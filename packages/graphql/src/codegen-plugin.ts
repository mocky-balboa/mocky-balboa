...
import { concatAST, type DocumentNode, type FragmentDefinitionNode, Kind, visit } from 'graphql';
...
const isDocumentNode = (document: unknown): document is DocumentNode => {
  return document !== undefined && document !== null && typeof document === "object" && "kind" in document && document.kind === Kind.DOCUMENT;
};
...
const plugin: CodegenPlugin["plugin"] = (schema, documents, config) => {
  const allDocuments = concatAST(documents.map(v => v.document).filter(isDocumentNode));
