import { type ASTNode, type ASTVisitFn, GraphQLSchema, Kind, type OperationDefinitionNode, OperationTypeNode } from 'graphql';
import {
...
export const mock${fqn} = (
  ...
  enter<TVisitedNode extends ASTNode>(...args: Parameters<ASTVisitFn<TVisitedNode>>) {
    const [node, _key, _parent, _path, _ancestors] = args;
    if (node.kind === Kind.OPERATION_DEFINITION && this.isSupportedOperation(node.operation) && node.name) {
