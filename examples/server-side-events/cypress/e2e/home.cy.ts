import type { SSE } from "@mocky-balboa/cypress";
...
it("updates the message in the UI when messages are received from the SSE endpoint", () => {
  ...
  cy.mockySSE(() => {
    ...
    cy.wrap(sse).as("sse");
    cy.then(() => sse.dispatchEvent("message", "This is a message from the server"));
    cy.contains("This is a message from the server");
    cy.then(() => sse.dispatchEvent("message", "Another message from the server"));
