    ...
    >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class:list={getDataCardClasses(!!nextFightError)}>
                ...
                {#if nextFightError || !nextFight}
                    <div
                        class="flex flex-col justify-center text-red-700 flex-1"
                    >
                        <p class="m-0">Failed to load next fight data</p>
                    </div>
                {:else}
                    <div
                        class="grid grid-cols-2 gap-y-4 text-left text-gray-700"
                    >
            ...
            <div class:list={getDataCardClasses(!!trainingRegimeError)}>
                ...
                {#if trainingRegimeError || !trainingRegime}
                    <div
                        class="flex flex-col justify-center text-red-700 flex-1"
                    >
                ...
                {:else}
                    <div
                        class="grid grid-cols-2 gap-y-4 text-left text-gray-700"
                    >
                        {#each Object.entries(trainingRegime.routine) as [label, value]}
