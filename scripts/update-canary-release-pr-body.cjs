...
const releaseJson = fs.readFileSync(path.join(rootDir, "release.json"), "utf8");
...
const prBody = response.data.body ?? "";
...
if (prBody.match(new RegExp(start))) {
  prBody = prBody.replace(new RegExp(`${start}.*?${end}`), releaseDescription);
} else {
  prBody = `${prBody}\n\n${releaseDescription}`;
}
